<template>
  ...
  export default {
    ...
    created() {
      console.log('ChatWidget component created');
    ...
    methods: {
      ...
      async initialize() {
        ...
        if (isLoggedIn) {
          console.log('User is logged in with ID:', userElement.dataset.userId);
          ...
          console.log('User is not logged in, checking for guest session');
          // Check for existing cookie
          this.sessionId = this.getCookie('chat_session_id');
          if (this.sessionId) {
            console.log('Found session ID in cookie:', this.sessionId);
            ...
            console.log('No session ID found, showing user form');
            this.showUserForm = true;
          }
        }
      },
      ...
      toggleChat() {
        ...
        console.log('Chat toggled:', this.isOpen ? 'open' : 'closed');
      ...
      async checkExistingConversation() {
        ...
        try {
          ...
          console.log('Conversation check response:', data);
          
          if (data.conversation) {
            console.log('Found existing conversation with ID:', data.conversation.id);
      ...
      async checkGuestSession() {
        ...
        this.loading = true;
        console.log('Checking guest session with ID:', this.sessionId);
        try {
          ...
          const data = await response.json();
          console.log('Guest session check response:', data);
          
          if (data.valid) {
            console.log('Valid guest session found with conversation ID:', data.conversation_id);
            ...
            console.log('Invalid guest session, resetting');
      ...
      async startChat() {
        if (!this.userInfo.name || !this.userInfo.email) {
          console.log('Name or email missing, cannot start chat');
          return;
        }
        
        this.loading = true;
        console.log('Starting guest chat with name:', this.userInfo.name);
        try {
          ...
          const data = await response.json();
          console.log('Guest chat initialization response:', data);
          
          if (data.status === 'success') {
            console.log('Chat successfully initialized with conversation ID:', data.conversation_id);
            this.conversationId = data.conversation_id;
      ...
      async sendMessage() {
        ...
        try {
          ...
          if (this.sessionId) {
            // Guest message
            console.log('Sending message as guest with session ID:', this.sessionId);
         ...
         console.log('Sending message as authenticated user with conversation ID:', this.conversationId);
       ...
       if (!this.conversationId && data.conversation_id) {
         console.log('Setting conversation ID from response:', data.conversation_id);
         this.conversationId = data.conversation_id;
       }
     } catch (error) {
   ...
   setupPusherForUser(userId) {
     if (!window.Echo) {
       console.error('Cannot setup Pusher: Echo not available');
       return;
     }
     
     console.log('Setting up Pusher for user with ID:', userId);
     
     window.Echo.private(`chat.${userId}`)
       .listen('NewChatMessage', (e) => {
         console.log('Received new message event via Pusher:', e);
   ...
   setupPusherForGuest() {
     if (!window.Echo || !this.sessionId) {
       console.error('Cannot setup Pusher for guest: Echo not available or session ID missing');
       return;
     }
     
     console.log('Setting up Pusher for guest with session ID:', this.sessionId);
     
     window.Echo.private(`chat.guest.${this.sessionId}`)
       .listen('NewChatMessage', (e) => {
         console.log('Received new message event via Pusher for guest:', e);
         // Add new message to the list
         this.messages.push(e.message);
         this.$nextTick(() => {
           this.scrollToBottom();
